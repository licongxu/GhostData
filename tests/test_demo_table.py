...
from ghostdata.tabular import feature_invariants, feature_score, load_table
...
def test_package_and_executor_job_are_table_agnostic(tmp_path: Path) -> None:
    ...
    assert task == {"label_column": "churned", "dataset": "dataset.csv"}
...
def test_run_table_demo_daytona_uses_injected_proposer_and_executor(
    ...
    fake_runner = LocalVerificationRunner(
        ...
        feature_score(reference, "churned", feature),
        "roc_auc",
    )
    ...
    assert observed_analysis == analysis
